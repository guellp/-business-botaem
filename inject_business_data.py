...
def download_and_parse_sheet():
    ...
    # 1. 기업마당 데이터 로드 및 파싱
    try:
        bizinfo_content = download_csv(BIZINFO_CSV_URL, "기업마당")
        lines = bizinfo_content.splitlines()
        reader = csv.DictReader(lines)
        rows = list(reader)
        print(f"   📋 기업마당 CSV 레코드 개수: {len(rows)}")
        ...
        for row in rows:
            prod_name = row.get("상품", "").strip()
            if not prod_name:
                continue
                
            raw_sido = row.get("지역", "").strip()
            norm_sido = get_norm_sido(raw_sido)
            
            raw_gugun = row.get("도시", "").strip()
            if raw_gugun in {"-", "", "전체", "전지역", "해당없음", "없음"}:
                norm_gugun = "전지역"
            else:
                norm_gugun = raw_gugun
                
            summary = row.get("사업개요", "").strip()
            
            method = row.get("사업신청방법", "").strip()
            site = row.get("사업신청 사이트", "").strip()
            if site:
                method += f"\n\n[신청 사이트] {site}"
                
            sector = "기타"
            if any(k in prod_name or k in summary for k in ["일자리", "고용", "취업", "채용"]):
                sector = "일자리"
            elif any(k in prod_name or k in summary for k in ["창업", "스타트업", "창출", "설립"]):
                sector = "창업"
            elif any(k in prod_name or k in summary for k in ["금융", "대출", "보증", "자금", "융자"]):
                sector = "금융"
            elif any(k in prod_name or k in summary for k in ["교육", "컨설팅", "멘토링", "상담"]):
                sector = "교육/컨설팅"
            elif any(k in prod_name or k in summary for k in ["수출", "판로", "마케팅", "홍보", "전시"]):
                sector = "마케팅/수출"
                
            support_type = "기타"
            if any(k in prod_name or k in summary for k in ["자금", "대출", "보증", "융자", "이자"]):
                support_type = "대출/보증/금융"
            elif any(k in prod_name or k in summary for k in ["지원금", "보조금", "수당", "교부"]):
                support_type = "보조금/현금지원"
            elif any(k in prod_name or k in summary for k in ["교육", "컨설팅", "특강", "세미나"]):
                support_type = "교육/컨설팅/기술지원"
        ...
        print(f"   📊 기업마당 데이터 파싱 성공: 총 {len(parsed_records)}건 추출 완료")
...
def auto_deploy():
    ...
    print("🌐 약 30초 후 사업 Botaem 사이트에 자동 반영됩니다!")
